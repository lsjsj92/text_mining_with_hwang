...
def go():
    ...
    for i in range(1,31):
        for j in range(1,50):
            ...
            url = 'https://news.naver.com/main/list.nhn?mode=LSD&mid=sec&sid1=100&date=201808'+date+'&page='+str(j)
            try:
                driver.get(url)
    ...
    for news in link:
        try:
            ...
            title_ = soup.find('h3', {'id': 'articleTitle'}).text
            content_ = soup.find('div', {'id': 'articleBodyContents'}).text
            date_ = soup.find('div', {'class': 'sponsor'}).find('span').text.split(" ")[0]
            title.append(title_)
