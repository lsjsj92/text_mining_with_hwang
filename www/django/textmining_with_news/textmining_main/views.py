...
class HwangMain(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            output = subprocess.check_output(['python3.6', './textmining_main/subprocess/subprocessed.py', value], stderr=subprocess.STDOUT)
        ...
        try:
            ...
            f = open("./hwang_main/log/subprocess_error.log", 'a', encoding='utf-8')
            f.write(log)
        with open('./hwang_main/subprocess/predict_result.txt', 'r', encoding='utf8') as f: category = f.read()
        pre = category
        cat = ''
        ...
        with open('./textmining_main/subprocess/tf_idf_json.json', 'r', encoding='utf8') as f:
            data = json.load(f)
        ...
        with open('./textmining_main/subprocess/tf_idf_json_test.json', 'w') as f:
            json.dump(tf_idf_data, f)
        #이제 이 키값과  word2vec 진행.
        w2v_model = word2vec.Word2Vec.load("./textmining_main/subprocess/word2vec.model")
